...
import { 
  ...
  Eye, XCircle, AlertTriangle, Send, ShieldAlert, History, Search, FileText, Smile, Check, MapPin
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [filtroRutaFlota, setFiltroRutaFlota] = useState("todas"); // 🎯 NUEVO: Filtro de rutas
  ...
  const [alertasParadas, setAlertasParadas] = useState([]); // 🎯 Actualizado a la tabla nueva
  const [editData, setEditData] = useState(null);
  
  // 🎯 Añadimos 'ruta' al estado inicial del formulario
  const [formData, setFormData] = useState({ 
    numero_unit: "", placa: "", capacidad: 4, ruta: "Maraven - Centro", 
    nom_chof: "", apellido_chof: "", ced_chof: "", tel_chof: "", mail_chof: "", pass_chof: "" 
  });
    ...
    try {
      if (modalType === "edit") {
        await supabase.from("choferes").update({ 
          ...
          ruta: editData.ruta // 🎯 Actualización de ruta
        }).eq("id", editData.id);
      } else {
        ...
        await supabase.from("choferes").insert([{ 
          ...
          apellido: formData.apellido_chof, 
          cedula: formData.ced_chof, 
          telefono: formData.tel_chof, 
          email: formData.mail_chof, 
          placa_vehiculo: formData.placa, 
          capacidad_total: parseInt(formData.capacidad, 10), 
          puestos_libres: parseInt(formData.capacidad, 10), 
          estado: 'disponible', 
          kyc_verificado: false,
          ruta: formData.ruta // 🎯 Inserción de ruta
        }]);
      }
      setModalType(null);
      await cargarDatos();
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-900 overflow-hidden text-left">
      ...
      <aside ref={sidebarRef} className="w-72 bg-[#0D47A1] text-white flex flex-col shrink-0 shadow-2xl z-30">
        ...
        <nav className="flex-1 px-6 space-y-3">
          ...
          <button onClick={() => setActiveTab("alertas")} className={`flex items-center justify-between w-full p-4 rounded-2xl font-black italic transition-all ${activeTab === "alertas" ? 'bg-white/10 shadow-lg border-l-4 border-red-400' : 'opacity-60 hover:opacity-100'}`}>
            <div className="flex items-center gap-4"><AlertTriangle size={20} className={alertasParadas.length > 0 ? "text-red-400" : ""} /> Paradas Llenas</div>
            {alertasParadas.length > 0 && <span className="bg-red-500 text-white font-bold text-[10px] px-2.5 py-1 rounded-full">{alertasParadas.length}</span>}
          </button>
        </nav>
      ...
      <main className="flex-1 shrink-0 flex flex-col h-screen overflow-hidden">
        <header className="h-24 bg-white border-b px-12 flex items-center justify-between shrink-0">
          <h1 className="text-2xl font-black italic uppercase tracking-tighter">{activeTab === "flota" ? "Tablero Control de Vehículos" : activeTab === "kyc" ? "Centro de Validación Digital (KYC)" : "🚨 Emergencias en Paradas"}</h1>
        ...
        <div className="flex-1 overflow-y-auto p-12 bg-slate-50/50">
          ...
          {activeTab === "flota" && (
            <div className="space-y-6">
              ...
              {/* 🎯 CONTROLES SUPERIORES FLOTA */}
              <div className="flex justify-between items-center bg-white p-4 rounded-3xl shadow-sm border border-slate-100">
                <div className="flex items-center gap-3 px-4">
                  <MapPin size={20} className="text-blue-500" />
                  <span className="text-xs font-black uppercase text-slate-500 tracking-wider">Filtrar por Ruta:</span>
                  <select 
                    value={filtroRutaFlota} 
                    onChange={(e) => setFiltroRutaFlota(e.target.value)} 
                    className="bg-slate-50 border border-slate-200 text-xs font-bold p-2.5 rounded-xl uppercase text-blue-700 outline-none cursor-pointer"
                  >
                ...
                <button onClick={() => setModalType("register")} className="flex items-center gap-2 bg-[#0D47A1] text-white px-5 py-3 rounded-2xl font-black italic uppercase text-xs hover:bg-blue-800 transition-all shadow-md"><Plus size={16}/> Alta Vehículo</button>
              </div>
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-50/50">
                    <tr className="text-[10px] uppercase font-black tracking-[0.2em] text-slate-400">
                      <th className="px-10 py-8">Vehículo</th>
                      <th className="px-10 py-8">Ruta Asignada</th>
                      <th className="px-10 py-8">Operador Encargado</th>
                      <th className="px-10 py-8">Estado</th>
                      <th className="px-10 py-8 text-right">Modificaciones</th>
                    </tr>
                  </thead>
                      ...
                      <React.Fragment key={u.id}>
                        <tr className="group hover:bg-slate-50/30">
                          ...
                          <td className="px-10 py-6">
                            <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider ${u.ruta === 'Maraven - Centro' ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'bg-emerald-50 text-emerald-700 border border-emerald-100'}`}>
                              {u.ruta || "SIN RUTA"}
                            </span>
                          </td>
                          ...
                          <td className="px-10 py-6 font-black text-slate-600 text-xs uppercase">{u.estado || "DISPONIBLE"}</td>
                        ...
                        {expandedRow === u.id && (
                          <tr className="bg-blue-50/30"><td colSpan="5" className="px-10 py-10 border-t border-blue-100/30"><div className="grid grid-cols-1 md:grid-cols-3 gap-6"><DetailCard icon={<ShieldCheck size={28}/>} label="Cédula Operador" value={`V-${u.cedula}`} color="blue"/><DetailCard icon={<Users size={28}/>} label="Capacidad" value={`${u.puestos_libres} / ${u.capacidad_total} ASIENTOS`} color="orange"/><DetailCard icon={<Clock size={28}/>} label="Última Hora Salida" value={u.hora_salida || "SIN RECORRIDO"} color="blue"/></div></td></tr>
                        )}
                      </React.Fragment>
                    ))}
                    {flotaFiltradaPorRuta.length === 0 && (
                      <tr><td colSpan="5" className="text-center py-10 text-slate-400 font-bold uppercase text-xs">No hay vehículos registrados en esta ruta.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
          ...
          {/* VISTA C: ALERTAS PARADAS (ACTUALIZADO A alertas_paradas) */}
          {activeTab === "alertas" && (
            <div className="bg-white rounded-[40px] shadow-xl border border-slate-100 overflow-hidden">
              <table className="w-full text-left"><thead className="bg-red-50/40"><tr className="text-[10px] uppercase font-black tracking-[0.2em] text-red-500"><th className="px-10 py-8">📍 Ubicación / Parada</th><th className="px-10 py-8">Reportado Por (Chequeador)</th><th className="px-10 py-8">Hora de Alerta</th><th className="px-10 py-8 text-right">Acción</th></tr></thead>
                <tbody className="divide-y divide-slate-50">
                  {alertasParadas.length > 0 ? (
                    alertasParadas.map(alerta => (
                      <tr key={alerta.id} className="bg-amber-50/10">
                        <td className="px-10 py-6"><span className="bg-red-100 text-red-700 font-black px-4 py-2 rounded-xl text-sm uppercase tracking-wide inline-block">{alerta.parada_nombre}</span></td>
                        <td className="px-10 py-6 font-black text-slate-700 uppercase text-xs">{alerta.chequeadores?.nombre} {alerta.chequeadores?.apellido} <span className="text-slate-400 block text-[10px]">V-{alerta.chequeadores?.cedula}</span></td>
                        <td className="px-10 py-6 text-xs font-medium text-slate-500">{new Date(alerta.creado_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</td>
                        <td className="px-10 py-6 text-right"><button onClick={() => setDispatchTarget(alerta)} className="inline-flex items-center gap-2 bg-emerald-500 text-white px-5 py-3 rounded-xl text-xs font-black uppercase hover:bg-emerald-600 transition-all shadow-md"><Car size={14} /> Atender</button></td>
                      </tr>
                    ))
                  ) : (
      ...
      {/* 🎯 MODAL UNIFICADO: ALTA / EDICIÓN DE FLOTA (CON RUTAS) */}
      {modalType && (
            ...
            {modalType === "edit" ? (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  {/* 🎯 EDITAR RUTA */}
                  <div>
                    <p className="text-[10px] text-blue-500 mb-1">Ruta Asignada</p>
                    <select value={editData?.ruta || "Maraven - Centro"} className="w-full bg-slate-50 border p-4 rounded-2xl outline-none text-slate-800" onChange={e => setEditData({...editData, ruta: e.target.value})} required>
                      <option value="Maraven - Centro">MARAVEN - CENTRO</option>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <p className="font-black text-[10px] text-blue-500">DATOS TÉCNICOS Y RUTA</p>
                  ...
                  {/* 🎯 SELECCIÓN DE RUTA AL CREAR */}
                  <select className="w-full bg-slate-50 border border-blue-200 p-4 rounded-2xl outline-none text-blue-700 bg-blue-50/30" onChange={e => setFormData({...formData, ruta: e.target.value})} required>
                    <option value="Maraven - Centro">RUTA: MARAVEN - CENTRO</option>
                    <option value="Maraven - Punta Cardón">RUTA: MARAVEN - P. CARDÓN</option>
                  </select>
                </div>
                <div className="space-y-4">
