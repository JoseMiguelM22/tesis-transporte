...
export default function Dashboard() {
  ...
  const handleLogout = async () => {
    ...
    // ✅ RUTA CORREGIDA AL HOME PRINCIPAL
    navigate("/");
  };
    ...
    <div className="min-h-screen bg-[#1566D0] font-sans text-white flex flex-col relative overflow-hidden text-left">
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white text-[#0D47A1] w-full max-w-sm rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="bg-emerald-500 p-8 text-white text-center flex flex-col items-center">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest opacity-90 mt-2">Pase de Abordaje Confirmado</p>
            </div>
            <div className="p-8 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 border-b border-gray-100 pb-5">
                ...
                <div><p className="text-[10px] font-black uppercase text-blue-400 mb-1">Hora Est.</p><p className="text-xl font-black italic leading-none">{ticketData.hora}</p></div>
              </div>
              <button onClick={() => setShowTicket(false)} className="w-full bg-[#0D47A1] text-white py-4 rounded-[20px] font-black text-xs uppercase tracking-widest shadow-lg active:scale-95 transition-all">Cerrar Ticket</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white text-[#0D47A1] w-full max-w-sm rounded-[45px] overflow-hidden shadow-2xl">
            <div className="bg-[#1566D0] p-8 pb-24 text-white relative">
              ...
              <div className="flex items-center gap-3 opacity-80 mb-4"><ShieldCheck size={18} /><span className="text-[10px] font-black uppercase tracking-widest">UNEFA Transporte</span></div>
            ...
            <div className="px-8 pb-10 -mt-20">
              <div className="relative inline-block mb-6">
                <div className="w-40 h-40 rounded-[38px] bg-gray-100 overflow-hidden border-4 border-white shadow-2xl flex items-center justify-center">
                  {showCamera ? <video ref={videoRef} autoPlay playsInline className="w-full h-full object-cover scale-x-[-1]" /> : userData.avatar_url ? <img src={userData.avatar_url} className="w-full h-full object-cover" /> : <User size={60} className="text-blue-200" />}
                ...
                {!showCamera ? (
                  <div className="absolute -bottom-2 -right-2 flex flex-col items-end">
                    {showPhotoOptions && (
                      <div className="bg-white rounded-2xl shadow-2xl p-2 mb-2 border border-gray-100 flex flex-col gap-1">
                        <button onClick={startCamera} className="flex items-center gap-2 px-4 py-2 hover:bg-blue-50 rounded-xl text-[10px] font-black uppercase"><Camera size={14} /> Cámara</button>
                    ...
                    <button onClick={() => setShowPhotoOptions(!showPhotoOptions)} className="bg-blue-600 text-white p-4 rounded-2xl shadow-xl hover:scale-110 transition-transform"><Camera size={20} /></button>
                  </div>
                ) : (
              ...
              <div className="space-y-6">
                {isEditing ? (
                  <div className="space-y-3">
                    <input className="w-full bg-gray-50 border-2 border-blue-50 rounded-xl px-4 py-2 font-bold outline-none" value={tempData.nombre} onChange={e => setTempData({...tempData, nombre: e.target.value})} />
                    <input className="w-full bg-gray-50 border-2 border-blue-50 rounded-xl px-4 py-2 font-bold outline-none" value={tempData.apellido} onChange={e => setTempData({...tempData, apellido: e.target.value})} />
                    <button onClick={handleUpdateNames} className="w-full bg-emerald-500 text-white py-3 rounded-xl font-black text-[10px] uppercase">{isSaving ? "Guardando..." : "Confirmar"}</button>
                  </div>
                ) : (
                  <>
                    <div><p className="text-[10px] font-bold uppercase text-blue-400 tracking-widest mb-1">Pasajero</p><h3 className="text-2xl font-black italic uppercase leading-none">{userData.nombre} {userData.apellido}</h3></div>
                    <button onClick={() => setIsEditing(true)} className="w-full bg-gray-50 text-[#0D47A1] py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2"><Edit2 size={14} /> Editar Datos</button>
                  </>
                )}
              </div>
            </div>
      ...
      {/* SIDEBAR */}
      <div className={`fixed inset-y-0 left-0 w-80 bg-[#0D47A1] z-50 transform ${isMenuOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-500 flex flex-col p-8`}>
        <div className="flex justify-between items-center mb-10">
          <CreditCard size={20} className="text-blue-300" />
          <button onClick={() => setIsMenuOpen(false)} className="p-3 bg-white/5 rounded-2xl"><X size={20} /></button>
        </div>
        <div className="bg-white/5 rounded-[32px] p-6 border border-white/10 mb-8 text-center">
          <div className="w-20 h-20 rounded-2xl bg-blue-500 mx-auto mb-4 overflow-hidden">
          ...
          <h3 className="font-black italic text-white uppercase truncate">{userData.nombre}</h3>
          <button onClick={() => { setIsProfileModalOpen(true); setIsMenuOpen(false); }} className="w-full mt-4 py-3 bg-white text-[#0D47A1] rounded-xl font-black text-[10px] uppercase">Ver Perfil</button>
        </div>
        <div className="flex-1 space-y-2">
          <button className="w-full flex items-center gap-4 p-4 rounded-2xl bg-white/5 font-black text-[10px] uppercase"><Car size={18} /> Mis Rutas</button>
          <button className="w-full flex items-center gap-4 p-4 rounded-2xl bg-white/5 font-black text-[10px] uppercase"><Bell size={18} /> Alertas</button>
        </div>
        {/* ✅ LOGOUT CORREGIDO */}
        <button onClick={handleLogout} className="flex items-center justify-center gap-3 w-full p-5 bg-red-500/10 rounded-[24px] font-black text-red-400 text-[10px] uppercase border border-red-500/20 hover:bg-red-500 hover:text-white transition-all"><LogOut size={16} /> Salir del Sistema</button>
      </div>

      {/* HEADER */}
      <header className="bg-[#0D47A1] pt-14 pb-10 px-8 rounded-b-[55px] flex justify-between items-center z-20 shadow-xl">
        <button onClick={() => setIsMenuOpen(true)} className="bg-white/10 p-3 rounded-2xl"><Menu size={24} /></button>
        <div className="text-right">
          <h1 className="text-2xl font-black italic uppercase leading-none">HOLA, {userData.nombre}</h1>
      ...
      <main className="flex-1 px-8 pt-8 pb-36 overflow-y-auto no-scrollbar">
        {/* MAPA DECORATIVO */}
        <div className="relative w-full h-64 bg-[#0a1d3d] rounded-[45px] shadow-2xl overflow-hidden mb-6">
          <iframe title="map" width="100%" height="100%" src="https://www.openstreetmap.org/export/embed.html?bbox=-70.2100%2C11.6700%2C-70.1600%2C11.7100&amp;layer=mapnik&amp;marker=11.6934%2C-70.1842" style={{ border: 0, filter: 'grayscale(1) contrast(1.2)' }}></iframe>
          <div className="absolute inset-0 bg-[#0D47A1]/40 flex flex-col items-center justify-center">
            <MapPin size={45} className="text-white animate-bounce" />
            <span className="mt-2 bg-[#0D47A1] px-4 py-1 rounded-full text-[10px] font-black uppercase border border-white/20">Sector Maraven</span>
          </div>
        </div>

        <button onClick={handleReporteParada} className="w-full mb-10 bg-orange-500/20 border border-orange-500/50 text-orange-400 py-4 rounded-[25px] font-black text-[10px] uppercase flex items-center justify-center gap-3 active:scale-95 transition-all">
          {enviandoReporte ? <Loader2 className="animate-spin" /> : <><AlertTriangle size={18} /> Reportar Parada Llena</>}
        </button>

        {selectedUnit ? (
          <div className="mb-10 bg-gradient-to-br from-[#2979FF] to-[#1566D0] rounded-[45px] p-9 shadow-2xl relative border border-white/10">
             ...
             <div className="relative z-10">
                ...
                <div className="flex gap-4">
                   ...
                   <div className="bg-white/15 px-5 py-3 rounded-2xl flex-1 text-center"><p className="text-[9px] font-black uppercase opacity-60 mb-1">Libres</p><p className="text-xl font-black italic">{selectedUnit.puestos_libres}</p></div>
                </div>
             </div>
          </div>
        ) : (
          <div className="mb-10 text-center text-blue-300 opacity-70 p-6 border-2 border-dashed border-blue-400/30 rounded-[30px]"><p className="text-xs font-bold uppercase italic">No hay unidades disponibles</p></div>
        )}

        <div className="space-y-4">
          <p className="text-[10px] font-black text-blue-200 uppercase tracking-[0.3em] ml-4 mb-4">Seleccionar Otra Unidad</p>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-[#1566D0] via-[#1566D0] to-transparent pt-16 z-30">
        <button onClick={handleReserva} disabled={loadingReserva || !selectedUnit} className="w-full bg-white text-[#1566D0] py-6 rounded-[32px] font-black text-xl shadow-2xl active:scale-95 uppercase italic flex items-center justify-center gap-4 disabled:opacity-70 transition-transform">
          {loadingReserva ? <Loader2 className="animate-spin" /> : <>Reservar Mi Asiento <ArrowRight size={24} /></>}
        </button>
      </div>
